...
void motor_init(uint8_t motor_id)
{
    ...
    DL_Timer_startCounter(PWMAB_INST); // 启动PWM计数器
}

//开PID
void motor_start_pid(void)
{   
...
int32_t limit_pid_duty(int32_t duty)
{
    if(duty > 4000){
        duty = 4000;
    }
...
void DC_MOTOR_PID(uint8_t motor_id)
{
    ...
    if (motor_id == 1) {
        ...
        PWM_1_duty = limit_pid_duty(PWM_1_duty);
    ...
    if (motor_id == 2) {
        ...
        PWM_2_duty = limit_pid_duty(PWM_2_duty);
